...
export default function TripCopilot({ trip, currency = 'USD' }: TripCopilotProps) {
  ...
  if (!isOpen) {
    return (
      <div
        ...
        className="flex items-center gap-4 bg-slate-900/40 p-4 rounded-3xl border border-slate-800 backdrop-blur-xl no-print cursor-pointer hover:bg-slate-900/60 hover:border-indigo-500/30 transition group"
      >
        <button
          className="w-14 h-14 shrink-0 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl shadow-[0_0_20px_rgba(79,70,229,0.2)] flex items-center justify-center text-3xl transition-transform group-hover:scale-105 border border-slate-800"
          title="Open AI Trip Co-pilot"
        >
          🤖
        </button>
        <div>
          <p className="text-sm font-bold text-white group-hover:text-indigo-300 transition-colors">AI Co-pilot</p>
          <p className="text-[11px] text-slate-400 mt-1 leading-tight">Clarify your doubts here or get quick trip advice!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      {/* Backdrop for mobile, optional, but let's just make it float */}
      <div
        id="copilot-panel"
        className="w-full bg-slate-950/90 backdrop-blur-xl border border-slate-800 rounded-3xl shadow-xl flex flex-col overflow-hidden h-[500px] animate-fadeIn no-print"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-indigo-950/80 to-purple-950/60 border-b border-slate-800 shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-800/60 transition ml-1"
              title="Close"
            >
